...
def send_mesage(user_id, message, keyboard=None):
  ...
  try:
      vk.method('messages.send', param)
       ...
       dataSerchParms[event.user_id] = {}
       print("Ошибка отправки данных VK! " + str(e))
...
def send_photo(user_id, list_of_ids, owner_id):
  for x in list_of_ids:
    try:
       ...
       dataSerchParms[event.user_id] = {}
       print("Ошибка загрузки фото VK! " + str(e))
       send_mesage(event.user_id, 'Ошибка ввода данных VK. Попробуйте еще раз. '+ str(e))
          ...
          try:
            ...
            if request == 'привет':
               try:
                 ...
                 dataSerchParms[event.user_id] = {}
                 print("Ошибка чтения значений пользователя! " + str(e))
                 send_mesage(event.user_id, 'Ошибка ввода данных VK. Попробуйте еще раз. '+ str(e))
              ...
              try:
                ...
                username= "Ошибка чтения имени пользователя!"
                print("Ошибка чтения имени пользователя! " + str(e))
                send_mesage(event.user_id, 'Ошибка ввода данных VK. Попробуйте еще раз. '+ str(e))
              send_mesage(event.user_id,
               ...
               try:
                   ...
                   print("Ошибка ввода данных VK. Попробуйте еще раз.! " + str(e))
                   send_mesage(event.user_id, 'Ошибка ввода данных VK. Попробуйте еще раз. '+ str(e))
            ...
            print("Глобальная ошибка сервера! " + str(e))
            send_mesage(event.user_id, 'Ошибка ввода данных VK. Попробуйте еще раз. '+ str(e))
